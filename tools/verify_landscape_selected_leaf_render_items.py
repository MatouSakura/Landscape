...
def main() -> int:
    ...
    checks = [
        ...
        ("terrain_first_index_location", lambda: require_contains(f("terrain_cpp"), r"GetTerrainRegionDrawIndexCount.*Region\.NumIndices.*Region\.MainNumIndices.*FirstIndexLocation\s*=.*Region\.FirstIndexLocation.*NumIndices\s*=\s*DrawIndexCount.*FirstIndexLocation\s*=\s*FirstIndexLocation.*BaseVertex\s*=\s*Region\.BaseVertex", "packed indexed region drawing")),
