...
def main() -> int:
    ...
    checks = [
        ...
        ("terrain_first_index_location", lambda: require_contains(f("terrain_cpp"), r"DrawIndexCount\s*=.*Region\.NumIndices.*Region\.MainNumIndices.*NumIndices\s*=\s*DrawIndexCount.*FirstIndexLocation\s*=\s*Region\.FirstIndexLocation.*BaseVertex\s*=\s*Region\.BaseVertex", "packed indexed region drawing")),
