...
def main() -> int:
    ...
    checks = [
        ...
        ("single_draw_uses_mesh_range", lambda: require_contains(f("terrain_cpp"), r"DrawAttrs\.NumIndices\s*=\s*Region\.NumIndices.*DrawAttrs\.FirstIndexLocation\s*=\s*Region\.FirstIndexLocation.*DrawAttrs\.BaseVertex\s*=\s*Region\.BaseVertex", "single indexed draw uses packed mesh range")),
