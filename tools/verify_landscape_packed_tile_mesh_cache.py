...
def main() -> int:
    ...
    checks = [
        ...
        ("single_draw_uses_mesh_range", lambda: require_contains(f("terrain_cpp"), r"DrawIndexCount\s*=.*Region\.NumIndices.*Region\.MainNumIndices.*DrawAttrs\.NumIndices\s*=\s*DrawIndexCount.*DrawAttrs\.FirstIndexLocation\s*=\s*Region\.FirstIndexLocation.*DrawAttrs\.BaseVertex\s*=\s*Region\.BaseVertex", "single indexed draw uses packed mesh range")),
