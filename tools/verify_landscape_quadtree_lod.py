...
def main() -> int:
    ...
    checks = [
        ...
        ("quadtree_selection", lambda: require_contains(f("quadtree_hpp"), r"struct\s+TerrainQuadtreeSelection.*SelectedNodeIndices.*MinSelectedLevel.*MaxSelectedLevel", "quadtree selection fields")),
        ("quadtree_api", lambda: require_contains(f("quadtree_hpp"), r"Build\s*\(.*Select\s*\(.*GetNodes\s*\(.*GetMaxDepth\s*\(.*GetSelectedLeafCapacity.*GetMaxSelectableLevel.*ComputeSplitDistance", "quadtree public API")),
        ...
        ("quadtree_split_rule", lambda: require_contains(f("quadtree_cpp"), r"ComputeSplitDistance.*SplitDistanceScale.*1u\s*<<\s*SafeLevel.*GetMaxSelectableLevel", "policy split-distance formula")),
